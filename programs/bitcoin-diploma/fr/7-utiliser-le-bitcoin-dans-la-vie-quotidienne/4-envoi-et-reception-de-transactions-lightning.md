...
Avec un portefeuille Lightning, utiliser Bitcoin est rapide, peu coûteux et privé, ce qui facilite les transactions entre deux personnes. Vous pouvez envoyer et recevoir du bitcoin rapidement pour des achats quotidiens comme prendre un café ou faire du shopping.

Voyons quelques exemples du Lightning Network en action.

###### Exemple 1

Marcia et Eve ont toutes les deux 5 unités de monnaie. Marcia souhaite envoyer 2 unités à Eve. Le paiement passe par Jeff, qui aide à faire transiter le paiement sur le Lightning Network. Une fois le paiement effectué, Eve a 7 unités et Marcia en a 3.
...
Les opérateurs de nœuds comme Jeff peuvent également gagner de petits frais pour l’acheminement des paiements. Ce faisant, ils contribuent à maintenir le réseau décentralisé et efficace.

Comparé aux transactions Bitcoin classiques :

* **Les transactions sur la chaîne** se produisent directement sur la blockchain Bitcoin. Elles sont très sécurisées mais peuvent être plus lentes et plus coûteuses.
* **Les transactions Lightning** se produisent hors chaîne et permettent aux paiements de circuler beaucoup plus rapidement et à un coût bien moindre.
...
Mina adore manger à l’extérieur et s’arrête souvent dans son café local préféré. Avec autant d’options de paiement différentes, elle ne sait pas laquelle est la meilleure. Heureusement, elle a appris un peu sur Bitcoin et le Lightning Network. Après avoir examiné ses options, Mina se rend compte qu’utiliser un moyen de paiement Lightning est la meilleure solution.
...
En termes simples, les transactions on-chain ont lieu directement sur la blockchain Bitcoin et peuvent prendre plus de temps et entraîner plus de frais. Les transactions Lightning se font hors chaîne, permettant des paiements rapides et peu coûteux tout en utilisant toujours le bitcoin.
...
**Il s'agit d'un exercice pratique où les étudiants envoient et reçoivent de vrais sats en utilisant le Réseau Lightning.**
...
1. Les paiements Lightning peuvent parfois être bloqués lors du routage, surtout pour les paiements importants. Bien que cela soit possible, ce genre d'expérience utilisateur devient de moins en moins fréquent à mesure que le réseau mûrit.

###### Astuce Étudiant

Vérifiez auprès de votre instructeur si et comment les frais de transaction Bitcoin on-chain actuels auront un impact sur le portefeuille Lightning que vous utilisez.
