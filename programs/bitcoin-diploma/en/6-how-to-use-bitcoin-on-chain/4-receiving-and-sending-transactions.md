...
>
> 1. **Copy and paste the address**: You can copy the address by highlighting it and pressing "Copy", then paste it into an email or message to the sender.
...
Once the sender has your (fresh) address, they can send you bitcoin by entering your address and the amount they want to send you and initiate the transaction. The bitcoin will then be deducated from the sender's wallet and will be visible in your wallet - once the transaction is confirmed on the Bitcoin Network. This takes 10 minutes, on average — though it is always advisable to wait for at least two confirmations (that is, for two blocks to be added to the blockchain), which will take on average 20 minutes.
...
>
> However, on-chain transactions are slower and can be significantly more expensive than otheroptions (which we will discuss in Chapter 7) due to the miner fee.
